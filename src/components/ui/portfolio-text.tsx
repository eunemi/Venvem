...
export function AnimatedLetterText({ text = "Portfolio", letterToReplace = "o", className }: AnimatedLetterTextProps) {
  ...
  if (replaceIndex === -1) {
    ...
    return <span className={cn("tracking-tight text-foreground", className)}>{text}</span>
  }
  ...
  return (
    <span className={cn("inline-flex items-center tracking-tight text-foreground", className)}>
